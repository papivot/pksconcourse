...
then
    ...
    echo "INFO - Got product slug: " $prod_slug
	 echo $prod_slug > $WORKDIR/prod_slug.txt
fi
...
echo "INFO - Getting the download links for: " $local_prod_name : $local_prod_version
...
export download_links=`curl --silent -H "Accept: application/json" -H "Content-Type: application/json" -H "Authorization: Token ${PIVNET_TOKEN}" \
        ...
        |jq -r -S '.product_files[]|select (.file_type == "Software")._links.download.href'`
...
file_version=`curl --silent -H "Accept: application/json" -H "Content-Type: application/json" -H "Authorization: Token ${PIVNET_TOKEN}" \
        -X GET https://network.pivotal.io/api/v2/products/${prod_slug}/releases/${release_id} \
        |jq -r -S '.product_files[]|select (.file_type == "Software").file_version'`

echo $file_version >> $WORKDIR/file_version
...
export download_names=`curl --silent -H "Accept: application/json" -H "Content-Type: application/json" -H "Authorization: Token ${PIVNET_TOKEN}" \
        -X GET https://network.pivotal.io/api/v2/products/${prod_slug}/releases/${release_id} \
        |jq -r -S '.product_files[]|select (.file_type == "Software").aws_object_key'`
